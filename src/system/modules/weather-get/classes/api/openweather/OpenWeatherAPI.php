	...
	protected function BuildURL(){
		...
		switch ($this->idType){
			case 'Unique':
				...
				$url= 'http://api.openweathermap.org/data/2.5/'.$type.'?id='.$this->place.'&appid='.$this->apikey.'&lang='.$this->language.'&units='.$this->unit.$cnt;
			break;
			case 'Name':
				//Try to Find City With Name
				$url= 'http://api.openweathermap.org/data/2.5/'.$type.'?q='.$this->place.'&appid='.$this->apikey.'&lang='.$this->language.'&units='.$this->unit.$cnt;
			break;
			case 'Coordinates':
				//lat / lon Coordinates

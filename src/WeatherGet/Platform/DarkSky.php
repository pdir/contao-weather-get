...
class DarkSky
{
    ...
    protected string $apiRoot = 'https://api.darksky.net/forecast/';
    public function __construct(string $key, string $place = "", string $language = "de", string $unit = "metric")
    {
        $this->apikey = $key;
        $this->place = $place;
    ...
    protected function BuildURL()
    {
        ...
        if (preg_match('/^(\-?\d+(\.\d+)?),\s*(\-?\d+(\.\d+)?)$/', $coordinates)) {
            $url = $this->apiRoot . $this->apikey . '/' . $coordinates . '?lang=' . $this->language . '&units=' . $unit . '&exclude=hourly';
        } else {
            $url = "error";
        }
